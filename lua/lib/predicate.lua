local Predicate = {}
local __Pred = { __index = Predicate }

function Predicate:new(f)
  return setmetatable({ f = f }, __Pred)
end

function Predicate:func(f)
  return setmetatable({f = function(_, _, ...) return f(...) end }, __Pred)
end

function __Pred:__call(...)
  return self:f(...)
end

function __Pred:__bnot()
  return self:new(function(...) return not self(...) end)
end

function __Pred:__band(q)
  return self:new(function(...) return self(...) and q(...) end)
end

function __Pred:__bor(q)
  return self:new(function(...) return self(...) or q(...) end)
end

function Predicate:gt(lim)
  return setmetatable({ lim = lim, f = function(self, _, x) return x > self.lim end }, __Pred)
end
...
function Predicate:eq(v)
  return setmetatable({ v = v, f = function(self, _, x) return x == self.v end }, __Pred)
end
...
function Predicate:in_range(low, high, low_open, high_open)
  ...
  return setmetatable(this, __Pred)
end
